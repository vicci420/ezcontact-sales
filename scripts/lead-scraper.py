...
def get_emails_from_url(url, already_sent, timeout=8):
    """Fetch URL and extract new emails."""
    try:
...
def scrape_site(base_url, vertical, already_sent):
    ...
    emails = get_emails_from_url(base_url, already_sent)
    if not emails:
        for path in CONTACT_PATHS:
            url = urljoin(base_url, path)
            emails = get_emails_from_url(url, already_sent)
            if emails:
                break
        time.sleep(0.5)
    return [(e, vertical, base_url) for e in emails]
...
def main():
    ...
    for i, (url, vertical) in enumerate(all_targets):
        ...
        if results:
            for e, v, u in results:
                seen_today.add(e)
                all_leads.append({"email": e, "vertical": v, "source": u})
                print(f"✅ {e}", end=" ")
            print()
