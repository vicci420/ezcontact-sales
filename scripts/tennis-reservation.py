...
def main():
    ...
    # Check for one-time date override
    if fecha in PARTNER_OVERRIDES:
        partner_name, partner_membership, _ = PARTNER_OVERRIDES[fecha]
        print(f"⚡ Override activo para {fecha}: usando {partner_name}")
    else:
    ...
    for cancha in court_priority:
        ...
        if "Se completo el apartado" in result:
            # Match folio: try multiple patterns (server response format varies)
            # Pattern 1: "Folio: 154857" or "Folio #154857"
            folio_match = re.search(r'Folio[:\s#]*(\d{5,})', result, re.IGNORECASE)
            if not folio_match:
                # Pattern 2: "No. 154857" or "Número 154857"
                folio_match = re.search(r'(?:N[oú][m.]?|Num\.?|Number)[:\s#]*(\d{5,})', result, re.IGNORECASE)
            if not folio_match:
                # Pattern 3: Any standalone 6-digit number in the success response
                folio_match = re.search(r'\b(1[5-9]\d{4})\b', result)  # 150000-199999 range
